...
| node\_role\_explicit\_self\_trust | Toggle for creating an explicit statement in the trust policy so that the Node Group IAM role can assume itself. | `bool` | `true` | no |
